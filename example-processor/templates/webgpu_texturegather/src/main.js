...
async function init(forceWebGL = false) {
  ...
  const camera = new OrthographicCamera(-aspect, aspect);
  camera.position.z = 2;
  ...
  rtScene.background = new Color(0x808080);
  const rtCamera = new PerspectiveCamera(
    50,
    1,
    cameraZ - 0.5 * Math.sqrt(3),
    cameraZ + 0.5 * Math.sqrt(3)
  );
  ...
  rtScene.add(new AmbientLight(0xffffff, 0.1));

  const box = new Mesh(
    new BoxGeometry(),
    new MeshStandardNodeMaterial({ color: 0xff0000 })
  );
