...
function createGUI() {
  ...
  sceneFolder
    .add(
      scene.userData,
      "timeRate",
      scene.userData.canGoBackwardsInTime ? -1 : 0,
      1
    )
    .name("Time rate");
  ...
  graphicsFolder
    .addColor(scene.userData, "lightningColorRGB")
    .name("Color")
    .onChange(function (value) {
      scene.userData.lightningMaterial.color
        .setRGB(value[0], value[1], value[2])
  ...
  graphicsFolder
    .addColor(scene.userData, "outlineColorRGB")
    .name("Glow color")
    .onChange(function (value) {
  ...
  rayFolder
    .add(scene.userData.rayParams, "straightness", 0, 1)
    .name("Straightness");
  rayFolder.add(scene.userData.rayParams, "roughness", 0, 1).name("Roughness");
  rayFolder
    .add(scene.userData.rayParams, "radius0", 0.1, 10)
    .name("Initial radius");
  rayFolder
    .add(scene.userData.rayParams, "radius1", 0.1, 10)
    .name("Final radius");
  rayFolder
    .add(scene.userData.rayParams, "radius0Factor", 0, 1)
    .name("Subray initial radius");
  rayFolder
    .add(scene.userData.rayParams, "radius1Factor", 0, 1)
    .name("Subray final radius");
  rayFolder
    .add(scene.userData.rayParams, "timeScale", 0, 5)
    .name("Ray time scale");
  rayFolder
    .add(scene.userData.rayParams, "subrayPeriod", 0.1, 10)
    .name("Subray period (s)");
  rayFolder
    .add(scene.userData.rayParams, "subrayDutyCycle", 0, 1)
    .name("Subray duty cycle");
  ...
  if (scene.userData.recreateRay) {
    ...
    raySlowFolder
      .add(scene.userData.rayParams, "ramification", 0, 15)
      .step(1)
      .name("Ramification")
    ...
    raySlowFolder
      .add(scene.userData.rayParams, "maxSubrayRecursion", 0, 5)
      .step(1)
      .name("Recursion")
      .onFinishChange(function () {
        scene.userData.recreateRay();
    ...
    raySlowFolder
      .add(scene.userData.rayParams, "recursionProbability", 0, 1)
      .name("Rec. probability")
      .onFinishChange(function () {
        scene.userData.recreateRay();
      });
...
function createConesScene() {
  ...
  const posLight = new PointLight(0x00ffff, 1, 5000, 2);
  scene.add(posLight);

  // Ground

  const ground = new Mesh(
    new PlaneGeometry(200000, 200000),
    new MeshPhongMaterial({ color: 0xc0c0c0, shininess: 0 })
  );
  ...
  const coneMesh1 = new Mesh(
    new ConeGeometry(coneHeight, coneHeight, 30, 1, false),
...
function createPlasmaBallScene() {
  ...
  const sphereMesh = new Mesh(
    new SphereGeometry(sphereRadius, 80, 40),
    sphereMaterial
  );
  sphereMesh.position.set(0, sphereHeight, 0);
  ballScene.add(sphereMesh);

  const sphere = new Sphere(sphereMesh.position, sphereRadius);

  const spherePlasma = new Mesh(
    new SphereGeometry(sphereRadius * 0.05, 24, 12),
    scene.userData.lightningMaterial
  );
  ...
  const box = new Mesh(
    new BoxGeometry(sphereHeight * 0.5, sphereHeight * 0.1, sphereHeight * 0.5),
    post.material
  );
...
function createStormScene() {
  ...
  const posLight = new PointLight(0x00ffff);
  ...
  const ground = new Mesh(
    ...
    new MeshLambertMaterial({ color: 0x072302 })
  );
  ground.rotation.x = -Math.PI * 0.5;
  scene.add(ground);
  ...
  // Black star mark
  const starVertices = [];
  const prevPoint = new Vector3(0, 0, 1);
  const currPoint = new Vector3();
  for (let i = 1; i <= 16; i++) {
    currPoint.set(
  ...
  const starMesh = new Mesh(
    starGeometry,
    new MeshBasicMaterial({ color: 0x020900 })
  );
