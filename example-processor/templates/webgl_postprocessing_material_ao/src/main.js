...
function init() {
  ...
  plyLoader.load("models/ply/binary/Lucy100k.ply", (geometry) => {
    ...
    const lucy = new Mesh(geometry, objectMaterial);
  ...
  const ambientLight = new AmbientLight(
    0xffffff,
    sceneParameters.ambientLightIntensity
  );
  const lightGroup = new Group();
  const planeGeometry = new PlaneGeometry(6, 6);
  const cylinderGeometry = new CylinderGeometry(0.5, 0.5, 1, 64);
  const sphereGeometry = new SphereGeometry(0.5, 32, 32);
  const lightSphereGeometry = new SphereGeometry(0.1, 32, 32);
  ...
  const pedestal = new Mesh(cylinderGeometry, groundMaterial);
  pedestal.position.set(0, 0.5, 0);
  pedestal.receiveShadow = true;
  pedestal.castShadow = true;
  scene.add(pedestal);
  const sphereMesh = new InstancedMesh(sphereGeometry, objectMaterial, 6);
  ...
  const lightSphereMesh = new InstancedMesh(
    lightSphereGeometry,
    emissiveMaterial,
    4
  );
  ...
  const updateOutput = () => {
    ...
    groundMaterial.aoPassMap =
      sceneParameters.output === 0 ? gtaoPass.gtaoMap : null;
    objectMaterial.aoPassMap =
      sceneParameters.output === 0 ? gtaoPass.gtaoMap : null;
  };
