...
function createPlaneStencilGroup(geometry, plane, stencilBit, renderOrder) {
  ...
  material.stencilFail = InvertStencilOp;
  material.stencilZFail = InvertStencilOp;
  material.stencilZPass = InvertStencilOp;
...
function init() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const poGroup = new ClippingGroup();
  ...
  const ground = new Mesh(
    ...
    new ShadowNodeMaterial({ color: 0x000000, opacity: 0.25, side: DoubleSide })
  );
  ...
  planeX
    .add(params.planeX, "constant", -1, 1)
  ...
  planeY
    .add(params.planeY, "constant", -1, 1)
  ...
  planeZ
    .add(params.planeZ, "constant", -1, 1)
