...
const sources = {
  millipede: {
    ...
    url: "three/examples/models/splat/millipede.splat",
    loader: SPLATLoader,
    rotation: new Euler(-Math.PI * -1.1, Math.PI * -0.1, Math.PI * 0.15),
    cameraPosition: new Vector3(0, 0.4, 1),
    credit: {
      author: "Fabian Plum",
      authorUrl: "https://superspl.at/user?id=scant3d",
...
function updateLicenseInfo(source) {
  ...
  license.innerHTML =
    ...
    'via <a href="' +
    credit.source +
    '" target="_blank" rel="noopener">superspl.at</a>. ' +
...
function fitCameraToSplats(mesh, source) {
  ...
  controls.minDistance = source.minDistance || radius * 1.1;
  controls.maxDistance = source.maxDistance || radius * 8;
  controls.target.copy(sphere.center).add(source.cameraTarget || zeroVector);
  camera.position
    .copy(source.cameraPosition)
    .normalize()
