...
async function init() {
  ...
  const lodTargets = [
    ...
    },
    {
      ratio: 0.55,
      error: 0.004,
      weights: [0.2, 0.2, 0.2, 0.35, 0.35],
      flags: ["RegularizeLight"],
    },
    {
      ratio: 0.25,
      error: 0.015,
      weights: [0.12, 0.12, 0.12, 0.2, 0.2],
      flags: ["RegularizeLight"],
    },
    {
      ratio: 0.1,
      error: 0.05,
      weights: [0.08, 0.08, 0.08, 0.12, 0.12],
      flags: ["RegularizeLight"],
    },
    {
      ratio: 0.04,
      error: 0.14,
      weights: [0.04, 0.04, 0.04, 0.06, 0.06],
      flags: ["Regularize", "Permissive"],
    },
    {
      ratio: 0.015,
      error: 0.3,
      weights: [0.02, 0.02, 0.02, 0.03, 0.03],
      flags: ["Regularize", "Permissive"],
    },
  ];
  ...
  const sourceIndices = geom.index
  ...
  console.info(
    "LOD Meshlets count: ",
    lods.map((l) => l.numChunks)
  );

  const totalVertices = lods.length * vertexCount;
  const totalIndices = totalChunks * 64 * 3;

  if (totalIndices / 3 > TRIANGLE_INDEX_MASK + 1)
    throw new Error("Triangle count exceeds payload bit budget");
  if (instanceCount > INSTANCE_INDEX_MASK + 1)
    throw new Error("Instance count exceeds payload bit budget");

  const maxTrianglesPerInstance = lods[0].numTriangles;
  const totalTriangles = instanceCount * maxTrianglesPerInstance;
  document.getElementById("triangleCount").innerText =
    new Intl.NumberFormat().format(totalTriangles);
    ...
    for (let m = 0; m < lod.numChunks; m++) {
      ...
      for (let t = 0; t < 64; t++) {
        ...
        if (t < meshletTriangles) {
          ...
          indexArray[triIdx * 3 + 0] =
            lod.vertexOffset + meshlet.vertices[a_local];
          indexArray[triIdx * 3 + 1] =
            lod.vertexOffset + meshlet.vertices[b_local];
          indexArray[triIdx * 3 + 2] =
            lod.vertexOffset + meshlet.vertices[c_local];
        } else {
  ...
  const lodOffsetsUniform = uniformArray(
    lods.map(
      (lod) =>
        new Vector4(lod.indexOffset / 3, lod.numTriangles, lod.chunkStart, 0)
    ),
    "vec4"
  );
  ...
  const vertexBuffer = storage(
    new StorageBufferAttribute(vertexArray, 4),
    "vec4",
    totalVertices
  ).toReadOnly();
  const normalBuffer = storage(
    new StorageBufferAttribute(normalArray, 4),
    "vec4",
    totalVertices
  ).toReadOnly();
  const uvBuffer = storage(
    new StorageBufferAttribute(uvArray, 2),
    "vec2",
    totalVertices
  ).toReadOnly();
  const indexBuffer = storage(
    new StorageBufferAttribute(indexArray, 1),
    "uint",
    totalIndices
  ).toReadOnly();
  const meshletIdBuffer = storage(
    new StorageBufferAttribute(meshletTriangleArray, 1),
    "uint",
    totalIndices / 3
  ).toReadOnly();
  ...
  parameterGroup.add(options, "Rasterizer", {
    "SW Only": "SW Only",
    "HW Only": "HW Only",
    Both: "Both",
  });
  ...
  const instanceDataAttr = new StorageBufferAttribute(staticInstanceData, 4);
  ...
  parameterGroup
    .add(options, "Grid", { XZ: "XZ", XYZ: "XYZ" })
    .addEventListener("change", updateGrid);

  parameterGroup
    .add(occlusionBiasUniform, "value", 0.0, 0.0008)
    .name("Occlusion Bias")
    .step(0.000001);

  parameterGroup
    .add(lodThresholdUniform, "value", 1, 15.0)
    .name("LOD Threshold")
    .step(0.1);
  ...
  const instancePrevWorldAttr = new StorageBufferAttribute(
    new Float32Array(instanceCount * 16),
    16
  );
  const instancePrevWorldBuffer = storage(
    instancePrevWorldAttr,
    "mat4",
    instanceCount
  );
  ...
  const hwQueueAttr = new StorageBufferAttribute(hwQueueData, 1);
  const hwQueueAtomic = storage(
    hwQueueAttr,
    "uint",
    1 + MAX_HW_TRIANGLES * 2
  ).toAtomic();
  const hwQueueRead = storage(
    hwQueueAttr,
    "uint",
    1 + MAX_HW_TRIANGLES * 2
  ).toReadOnly();
  ...
  prevProjScreenUniform = uniform(new Matrix4());
  ...
  prevCameraPosUniform = uniform(new Vector3());
    ...
    hzbKernels.push(
      Fn(() => {
        ...
        If(instanceIndex.lessThan(levelWidth.mul(levelHeight)), () => {
            ...
            for (let dy = 0; dy < 2; dy++) {
              for (let dx = 0; dx < 2; dx++) {
                depthMax.assign(
                  max(
                    depthMax,
                    depthSourceTexNode.load(
                      uvec2(min(sx.add(dx), sw), min(sy.add(dy), sh))
                    ).r
                  )
                );
              }
            ...
            for (let dy = 0; dy < 2; dy++) {
              for (let dx = 0; dx < 2; dx++) {
                ...
                depthMax.assign(
                  max(
                    depthMax,
                    hzbBuffer.element(srcOffset.add(ty.mul(srcWidth)).add(tx))
                  )
                );
              }
            }
          ...
          hzbBuffer
            .element(levelOffset.add(y.mul(levelWidth)).add(x))
            .assign(depthMax);
        });
      })()
        .compute(initialInfo.y * initialInfo.z, [64])
        .setName(`HZB Level ${k}`)
    );
  }
  ...
  const sphereOccluded = (center, radius) => {
    ...
    const maxZ = max(
      max(
        hzbRead.element(levelOffset.add(y0.mul(levelWidth)).add(x0)),
        hzbRead.element(levelOffset.add(y0.mul(levelWidth)).add(x1))
      ),
      max(
        hzbRead.element(levelOffset.add(y1.mul(levelWidth)).add(x0)),
        hzbRead.element(levelOffset.add(y1.mul(levelWidth)).add(x1))
      )
    );
    ...
    return dist
      .greaterThan(radius.mul(2.0)) // skip spheres close to the camera
  ...
  computeClear = Fn(() => {
    ...
    .compute(maxPixels, [256])
    .setName("Compute Clear");
  ...
  computeFrustum = Fn(() => {
    ...
    instancePrevWorldBuffer
      ...
      .assign(instanceWorldBuffer.element(instanceIndex));
    ...
    If(visible, () => {
      ...
      const pixelFactor = cotHalfFovUniform
      ...
      for (let i = lods.length - 1; i > 0; i--) {
        const checkLod = float(lods[i].error)
          .mul(scale)
          .mul(pixelFactor)
      ...
      Loop(
        ...
        ({ cIdx: chunkIndex }) => {
          ...
          const chunkCenterWorld = matrixWorld
            .mul(vec4(chunkCenterLocal, 1.0))
            .xyz.toVar();
          ...
          If(chunkVisible, () => {
            const chunkCenterPrev = instancePrevWorldBuffer
              .element(instanceIndex)
              .mul(vec4(chunkCenterLocal, 1.0))
              .xyz.toVar();
            chunkVisible.assign(
              sphereOccluded(chunkCenterPrev, chunkRadiusWorld).not()
            );
          });
      ...
      instanceMvpBuffer
        .element(instanceIndex)
        .assign(projScreenMatrixUniform.mul(matrixWorld));
    });
  })()
    .compute(instanceCount)
    .setName("Compute Frustum");
  ...
  computeRasterize = Fn(() => {
    ...
    If(instanceIndex.lessThan(totalThreads), () => {
      ...
      If(globalTriangleIndex.lessThan(lodNumTriangles), () => {
        ...
        If(
          p0.w
            .greaterThan(0.0)
            .and(p1.w.greaterThan(0.0))
            .and(p2.w.greaterThan(0.0)),
          () => {
                ...
                () => {
                  ...
                  If(
                    startX
                      ...
                      .and(bbWidth.lessThanEqual(maxRasterSizeUniform))
                      .and(bbHeight.lessThanEqual(maxRasterSizeUniform)),
                    () => {
                      ...
                      const isTopLeft0 = stepX_w0
                        .lessThan(0.0)
                        .or(stepX_w0.equal(0.0).and(stepY_w0.greaterThan(0.0)));
                      const isTopLeft1 = stepX_w1
                        .lessThan(0.0)
                        .or(stepX_w1.equal(0.0).and(stepY_w1.greaterThan(0.0)));
                      const isTopLeft2 = stepX_w2
                        .lessThan(0.0)
                        .or(stepX_w2.equal(0.0).and(stepY_w2.greaterThan(0.0)));
                      ...
                      const pStart = vec2(
                        float(startX).add(0.5),
                        float(startY).add(0.5)
                      );
                            ...
                            ({ x }) => {
                              If(
                                w0
                                  ...
                                  .and(w1.greaterThanEqual(0.0))
                                  .and(w2.greaterThanEqual(0.0)),
                                () => {
                                  If(
                                    z
                                      .greaterThanEqual(0.0)
                                      .and(z.lessThanEqual(1.0)),
                                    () => {
                                      // Depth (fourth-root distribution) packed above each payload's bits
                                      const zEncoded = sqrt(
                                        sqrt(float(1.0).sub(z))
                                      );
                                      const depthTri = uint(
                                        zEncoded.mul(DEPTH_TRI_MAX)
                                      );
                                      const depthInst = uint(
                                        zEncoded.mul(DEPTH_INST_MAX)
                                      );

                                      const packedTri = depthTri
                                        .shiftLeft(TRIANGLE_INDEX_BITS)
                                        .bitOr(
                                          megaTriangleIndex.bitAnd(
                                            TRIANGLE_INDEX_MASK
                                          )
                                        );
                                      const packedInst = depthInst
                                        .shiftLeft(INSTANCE_INDEX_BITS)
                                        .bitOr(instId);

                                      const pixelIndex = uint(y)
                                        .mul(uint(screenSize.x))
                                        .add(uint(x));

                                      // Early depth pre-check: skip the atomics if the pixel already has a closer fragment
                                      const currentDepth = atomicLoad(
                                        screenTriAtomic.element(pixelIndex)
                                      ).shiftRight(TRIANGLE_INDEX_BITS);
                                      If(
                                        depthTri.greaterThanEqual(currentDepth),
                                        () => {
                    ...
                    If(
                      ...
                      () => {
                        ...
                        If(hwCount.lessThan(MAX_HW_TRIANGLES), () => {
                          ...
                          atomicStore(
                            hwQueueAtomic.element(hwSlot.add(1)),
                            megaTriangleIndex
                          );
                        });
                      }
    ...
    .compute(dispatchAttr)
    .setName("Compute Rasterize");
  ...
  const applyNormalMap = (normal, tangent, mapSample) => {
    ...
    return normalize(
      tangent.mul(mapN.x).add(bitangent.mul(mapN.y)).add(normal.mul(mapN.z))
    );
  };
  ...
  // It renders AFTER the fullscreen resolve, overlaying HW-rasterized triangles.
  {
    ...
    hwGeometry.setAttribute(
      "position",
      new Float32BufferAttribute(new Float32Array(MAX_HW_TRIANGLES * 3 * 3), 3)
    );
    hwGeometry.setIndirect(hwDrawAttr);
    hwGeometry.boundingSphere = new Sphere().set(new Vector3(), Infinity);
    ...
    const hwPosition = Fn(() => {
      ...
      const worldPos = localVert
        .equal(1)
        .select(w1, localVert.equal(2).select(w2, w0));

      const worldNormal = normalize(
        matrixWorld.mul(vec4(normalBuffer.element(vertGlobalIdx).xyz, 0.0)).xyz
      );

      const uv0 = uvBuffer.element(i0);
      const uv1 = uvBuffer.element(i1);
      const uv2 = uvBuffer.element(i2);
      const uvVal = localVert
        .equal(1)
        .select(uv1, localVert.equal(2).select(uv2, uv0));
    ...
    const hwKernelRoughness = min(
      hwDNdx.dot(hwDNdx).add(hwDNdy.dot(hwDNdy)).mul(SPECULAR_AA_VARIANCE),
      SPECULAR_AA_MAX
    );

    const hwShadedMaterial = new MeshStandardNodeMaterial();
    hwShadedMaterial.positionNode = hwPosition;
    hwShadedMaterial.colorNode = sampleMapHW(sourceMaterial.map);
    hwShadedMaterial.normalNode = applyNormalMap(
      hwNormal,
      normalize(vTangent),
    ...
    hwMesh = new Mesh(hwGeometry, hwShadedMaterial);
  ...
  // lighting pipeline (environment + lights) can shade the visibility buffer.
  {
    ...
    resolveGeometry.boundingSphere = new Sphere().set(new Vector3(), Infinity);
    ...
    const pixelIndex = uint(flippedY)
      .mul(uint(screenSize.x))
      .add(uint(screenCoordinate.x));

    const packedTri = screenTriRead.element(pixelIndex);
    const megaTriangleIndex = packedTri.bitAnd(TRIANGLE_INDEX_MASK);
    const instId = screenInstRead
      .element(pixelIndex)
      .bitAnd(INSTANCE_INDEX_MASK);
    ...
    const normal_interp = normalize(
      n0.mul(b0_p).add(n1.mul(b1_p)).add(n2.mul(b2_p))
    );

    const worldPosition = w0.mul(b0_p).add(w1.mul(b1_p)).add(w2.mul(b2_p));
    const positionViewHelmet = cameraViewMatrix.mul(
    ...
    const kernelRoughness = min(
      dNdx.dot(dNdx).add(dNdy.dot(dNdy)).mul(SPECULAR_AA_VARIANCE),
      SPECULAR_AA_MAX
    );

    // Discard pixels the rasterizer did not cover so the background shows through
    const coveredColor = (colorNode) =>
      Fn(() => {
    ...
    const resolveShadedMaterial = new MeshStandardNodeMaterial();
    resolveShadedMaterial.contextNode = overrideNodes([
    ...
    resolveShadedMaterial.colorNode = coveredColor(
      sampleMap(sourceMaterial.map)
    );
    ...
    resolveShadedMaterial.emissiveNode = sampleMap(
      sourceMaterial.emissiveMap
    ).rgb;

    // Meshlet debug: flat colors per cluster
    const resolveDebugMaterial = new NodeMaterial();
    resolveDebugMaterial.vertexNode = fullscreenVertex;
    resolveDebugMaterial.depthNode = resolveDepth;
    resolveDebugMaterial.fragmentNode = coveredColor(
      hashColor(
        meshletIdBuffer.element(megaTriangleIndex).add(instId.mul(1000))
      )
    );

    // Vis material: unlit visualization of channels
    const resolveVisMaterial = new NodeMaterial();
    ...
    resolveVisMaterial.fragmentNode = coveredColor(
      getVisColor(
        outputModeUniform,
        normal_interp,
        applyNormalMap(
    ...
    resolveMesh = new Mesh(resolveGeometry, resolveShadedMaterial);
    ...
    blitQuad = new QuadMesh(blitMaterial);
  }
...
function createScreenBuffers() {
  ...
  screenTriAttr = new StorageBufferAttribute(screenTriData, 1);

  const screenInstData = new Uint32Array(maxPixels);
  screenInstAttr = new StorageBufferAttribute(screenInstData, 1);
  ...
  sceneRT = new RenderTarget(size.x, size.y, { type: HalfFloatType });
  sceneRT.depthTexture = new DepthTexture(size.x, size.y);
  sceneRT.depthTexture.type = FloatType;
  ...
  while (hzbLevelCount < MAX_HZB_LEVELS) {
    hzbLevelTable.array[hzbLevelCount].set(
      totalTexels,
      levelWidth,
      levelHeight,
      0
  ...
  const hzbAttr = new StorageBufferAttribute(hzbData, 1);
...
const prevProjScreen = new Matrix4();
