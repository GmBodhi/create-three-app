...
// Data shared between the Meshes on the bottom and WebXR Layers on the top for the eye
...
function init() {
  ...
  const eyeCharts = new Group();
  eyeCharts.position.z = snellenConfig.z;
  scene.add(eyeCharts);

  snellenTexture = new TextureLoader().load("textures/snellen.png");
  ...
  snellenTexture.minFilter = LinearFilter;
  const snellenMeshPlain = new Mesh(
    new PlaneGeometry(snellenConfig.widthMeters, snellenConfig.heightMeters),
    new MeshBasicMaterial({ map: snellenTexture })
  );
  ...
  snellenTexture.repeat.x = snellenConfig.cropX;
  snellenTexture.repeat.y = snellenConfig.cropY;
  const snellenMeshMipMap = new Mesh(
    new PlaneGeometry(snellenConfig.widthMeters, snellenConfig.heightMeters),
    new MeshBasicMaterial({ map: snellenTexture })
  );
  ...
  errorMesh = new Mesh(
    ...
    new MeshBasicMaterial({ map: new CanvasTexture(errorCanvas) })
  );
