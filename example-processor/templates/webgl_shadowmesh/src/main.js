...
const arrowPosition1 = new Vector3();
const arrowPosition2 = new Vector3();
const arrowPosition3 = new Vector3();
let groundMesh;
...
const groundPlane = new Plane(normalVector, planeConstant);
...
function init() {
  ...
  // LIGHTBULB
  const lightSphereGeometry = new SphereGeometry(0.09);
  ...
  const lightHolderGeometry = new CylinderGeometry(0.05, 0.05, 0.13);
  const lightHolderMaterial = new MeshBasicMaterial({ color: "rgb(75,75,75)" });
  lightHolder = new Mesh(lightHolderGeometry, lightHolderMaterial);
  scene.add(lightHolder);
  lightHolder.visible = false;

  // GROUND
  const groundGeometry = new BoxGeometry(30, 0.01, 40);
  const groundMaterial = new MeshLambertMaterial({ color: "rgb(0,130,0)" });
  ...
  const cylinderGeometry = new CylinderGeometry(0.3, 0.3, 2);
  ...
  const torusGeometry = new TorusGeometry(1, 0.2, 10, 16, TWO_PI);
  ...
  const sphereGeometry = new SphereGeometry(0.5, 20, 10);
  ...
  const pyramidGeometry = new CylinderGeometry(0, 0.5, 2, 4);
  const pyramidMaterial = new MeshPhongMaterial({
    color: "rgb(255,255,0)",
    emissive: 0x440000,
    flatShading: true,
    shininess: 0,
