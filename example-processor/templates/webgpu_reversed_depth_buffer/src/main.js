...
async function init() {
  ...
  normalRenderer = new WebGPURenderer();
  ...
  logarithmicRenderer = new WebGPURenderer({ logarithmicDepthBuffer: true });
  ...
  reverseRenderer = new WebGPURenderer({ reversedDepthBuffer: true });
  ...
  normalRenderPipeline = new RenderPipeline(normalRenderer);
  normalRenderPipeline.outputNode = pass(scene, camera);

  logarithmicRenderPipeline = new RenderPipeline(logarithmicRenderer);
  logarithmicRenderPipeline.outputNode = pass(scene, camera);

  reverseRenderPipeline = new RenderPipeline(reverseRenderer);
  reverseRenderPipeline.outputNode = pass(scene, reversedCamera);

  await Promise.all([
    normalRenderer.init(),
    logarithmicRenderer.init(),
    reverseRenderer.init(),
  ]);
