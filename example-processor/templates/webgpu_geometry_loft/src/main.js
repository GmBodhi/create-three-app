...
async function init() {
  ...
  const background = screenUV
    .distance(0.5)
    .mix(color(0x5d5d84), color(0x2e2e44));
  ...
  const floorMaterial = new MeshStandardNodeMaterial({ roughness: 1 });
  floorMaterial.colorNode = color(0x555577).mul(
    mx_noise_float(positionLocal.mul(0.4)).mul(0.1).add(0.95)
  );

  const floor = new Mesh(
    new CircleGeometry(58, 64).rotateX(-Math.PI / 2),
    floorMaterial
  );
  ...
  const curtainMaterial = new MeshStandardNodeMaterial({
    roughness: 0.9,
    side: DoubleSide,
  });
  curtainMaterial.colorNode = color(0x86222e).mul(
    mx_noise_float(vec3(uv().y.mul(300), uv().x.mul(6), 0))
      .mul(0.08)
      .add(0.96)
  );

  const curtain = new Mesh(createCurtainGeometry(), curtainMaterial);
  group.add(curtain);
  ...
  const vein = mx_fractal_noise_float(
    p.add(mx_fractal_noise_float(p.mul(0.4), 3).mul(2)),
    4
  )
    .abs()
    .oneMinus();
  ...
  const veining = vein
    .pow(4)
    .mul(0.3)
    .add(vein.pow(12).mul(0.7))
    .add(fine.pow(14).mul(0.2));
  const clouds = mx_noise_float(p.mul(0.5)).mul(0.5).add(0.5);

  const pedestalMaterial = new MeshStandardNodeMaterial();
  pedestalMaterial.colorNode = mix(
    mix(color(0xf4f4f7), color(0xeeeef2), clouds),
    color(0xd0d0d5),
    veining
  );
  ...
  function addPedestal(x, z, radius, height) {
    const pedestal = new Mesh(
      createPedestalGeometry(radius, height),
      pedestalMaterial
    );
  ...
  const porcelain = new MeshStandardNodeMaterial();
  porcelain.roughnessNode = mx_noise_float(positionLocal.mul(6))
    .mul(0.08)
    .add(0.2);
  porcelain.normalNode = bumpMap(
    mx_noise_float(positionLocal.mul(2)).mul(0.05)
  );
  ...
  const saucer = new Mesh(createSaucerGeometry(), porcelain);
  coffee.add(saucer);

  const cup = new Mesh(createCupGeometry(), porcelain);
  cup.position.y = 0.3;
  coffee.add(cup);

  const handle = new Mesh(createHandleGeometry(), porcelain);
  ...
  const liquidMaterial = new MeshStandardNodeMaterial({ roughness: 0.08 });
  liquidMaterial.colorNode = mix(
    color(0x2b1a12),
    color(0x4a2c1a),
    mx_noise_float(positionLocal.mul(1.5)).mul(0.5).add(0.5)
  );

  const liquid = new Mesh(
    new CircleGeometry(2, 48).rotateX(-Math.PI / 2),
    liquidMaterial
  );
  ...
  const vaseMaterial = new MeshStandardNodeMaterial({ side: DoubleSide });
  vaseMaterial.colorNode = mix(
    color(0x2e6f9e),
    color(0x82b8d8),
    mx_noise_float(positionLocal.mul(1.2)).mul(0.5).add(0.5)
  );
  vaseMaterial.roughnessNode = vaseRings.mul(0.08).add(0.3);
  vaseMaterial.normalNode = bumpMap(vaseRings.mul(0.012));

  const vase = new Mesh(createVaseGeometry(), vaseMaterial);
  ...
  const shell = new Mesh(createShellGeometry(), shellMaterial);
  ...
  const starMaterial = new MeshStandardNodeMaterial({ roughness: 0.6 });
  starMaterial.colorNode = color(0xcc5544).mul(
    mx_noise_float(positionLocal.mul(3)).mul(0.12).add(0.94)
  );
  starMaterial.normalNode = bumpMap(
    mx_noise_float(positionLocal.mul(50)).mul(0.008)
  );

  const star = new Mesh(createStarGeometry(), starMaterial);
  ...
  const ribbonMaterial = new MeshStandardNodeMaterial({
    color: 0xffcc44,
    metalness: 1,
    side: DoubleSide,
  });
  ...
  const ribbon = new Mesh(createRibbonGeometry(), ribbonMaterial);
  ...
  const tubeU = uv().x;
  const tealStripe = smoothstep(0.48, 0.5, tubeU).sub(
    smoothstep(0.6, 0.62, tubeU)
  );
  const redStripe = smoothstep(0.66, 0.68, tubeU).sub(
    smoothstep(0.72, 0.74, tubeU)
  );

  const tubeMaterial = new MeshStandardNodeMaterial({ roughness: 0.25 });
  tubeMaterial.colorNode = mix(
    mix(color(0xf2f2f2), color(0x2aa6b8), tealStripe),
    color(0xd0543a),
    redStripe
  );

  const tube = new Mesh(createToothpasteGeometry(), tubeMaterial);
  ...
  const pumpkinMaterial = new MeshStandardNodeMaterial();
  ...
  pumpkinMaterial.normalNode = bumpMap(
    mx_noise_float(vec3(uv().y.mul(120), uv().x.mul(5), 0)).mul(0.01)
  );

  const pumpkinY = addPedestal(0, 7.8, 2, 1.6);

  const pumpkin = new Mesh(createPumpkinGeometry(), pumpkinMaterial);
  pumpkin.position.set(0, pumpkinY, 7.8);
  group.add(pumpkin);

  const pumpkinStemMaterial = new MeshStandardNodeMaterial({ color: 0x667744 });
  pumpkinStemMaterial.roughnessNode = mx_noise_float(positionLocal.mul(20))
    .mul(0.2)
    .add(0.7);

  const pumpkinStem = new Mesh(
  ...
  const warts = smoothstep(
    0.18,
    0.38,
    mx_worley_noise_float(positionLocal.mul(2.4))
  )
    .oneMinus()
    .mul(dome);
  ...
  const mushroomCap = new Mesh(createMushroomCapGeometry(), capMaterial);
  mushroomCap.position.set(-7.8, mushroomY, 0);
  group.add(mushroomCap);

  const mushroomStemMaterial = new MeshStandardNodeMaterial();
  mushroomStemMaterial.colorNode = color(0xe5d5b5).mul(
    mx_noise_float(vec3(uv().y.mul(24), uv().x.mul(2), 0))
      .mul(0.1)
      .add(0.94)
  );
  mushroomStemMaterial.roughnessNode = mx_noise_float(positionLocal.mul(12))
    .mul(0.15)
    .add(0.55);

  const mushroomStem = new Mesh(
  ...
  const gobletMaterial = new MeshStandardNodeMaterial({
    color: 0xb87333,
    metalness: 1,
  });
  ...
  const goblet = new Mesh(createGobletGeometry(), gobletMaterial);
  ...
  const brassMaterial = new MeshStandardNodeMaterial({
    color: 0xc9a86a,
    metalness: 1,
  });
  brassMaterial.roughnessNode = mx_noise_float(positionLocal.mul(10))
    .mul(0.06)
    .add(0.16);
  brassMaterial.envMapIntensity = 2;

  const ropeMaterial = new MeshStandardNodeMaterial({
    color: 0x8a2433,
    roughness: 0.65,
  });
  ropeMaterial.normalNode = bumpMap(
    sin(
      uv()
        .x.mul(200)
  ...
  const ropeGeometry = createRopeGeometry(
    2 * barrierRadius * Math.sin(Math.PI / posts)
  );

  for (let i = 0; i < posts; i++) {
    const angle = ((i + 0.5) / posts) * Math.PI * 2;

    const post = new Mesh(stanchionGeometry, brassMaterial);
    post.position.set(
      Math.sin(angle) * barrierRadius,
      -5,
      Math.cos(angle) * barrierRadius
    );
    group.add(post);
    ...
    rope.position.set(
      Math.sin(mid) * midRadius,
      -5 + 2.05,
      Math.cos(mid) * midRadius
    );
    rope.rotation.y = mid;
    group.add(rope);
  }
  ...
  group.traverse((child) => {
    if (child.isMesh && child.geometry.type === "LoftGeometry")
      lofts.push(child);
  });
  ...
  for (const mesh of lofts) {
    ...
    if ((sections.length - 1) % step !== 0)
      addRing(sections[sections.length - 1]);
  ...
  for (let i = 0; i <= 150; i++) {
    ...
    for (let j = 0; j < 32; j++) {
      ...
      points.push(
        new Vector3(
          r * sin,
          4.5 * (1 - e) + sectionRadius * Math.sin(phi),
          r * cos
        )
      );
    }
...
function createRibbonGeometry() {
  ...
  for (let i = 0; i <= 120; i++) {
    ...
    sections.push([
      new Vector3(3 * sin, t * 7.5, 3 * cos),
      new Vector3(3 * sin, t * 7.5 + 2, 3 * cos),
    ]);
  }
...
function createToothpasteGeometry() {
  ...
  for (let i = 0; i <= 80; i++) {
    ...
    for (let j = 0; j < 48; j++) {
      const angle = (j / 48) * Math.PI * 2;
      points.push(
        new Vector3(Math.sin(angle) * width, t * 4.2, Math.cos(angle) * depth)
      );
    }
...
function createPumpkinStemGeometry() {
  ...
  for (let i = 0; i <= 30; i++) {
    ...
    for (let j = 0; j < 32; j++) {
      ...
      const r =
        radius * (0.92 + 0.13 * Math.pow(Math.abs(Math.cos(2.5 * angle)), 0.5));

      points.push(
        new Vector3(
          lean + Math.sin(angle) * r,
          1.3 + 1.15 * t,
          Math.cos(angle) * r
        )
      );
