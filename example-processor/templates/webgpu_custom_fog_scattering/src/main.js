...
async function init() {
  ...
  scene.fog = new FogExp2(0xcccccc, 0.014);
  ...
  const buildWindows = positionWorld.y
    ...
    .mod(4)
    .sign()
    .mix(color(0x000066), color(0xffffff));
  ...
  scene.add(new HemisphereLight(0xf0f5f5, 0xd0dee7, 0.5));
  ...
  const sceneColorBlurred = gaussianBlur(scenePassColor, vec2(scattering), 4, {
    resolutionScale: 0.5,
  });

  // composite

  const fogFactor = densityFogFactor(density).context({
