...
const uVolumeWorldSize = uniform(
...
const prevTeapotPos = new Vector3();
...
function createComputePasses() {
  ...
  computeCurlNoisePass = Fn(() => {
    ...
    const p = uvw.mul(
      vec3(
        VOLUME_WORLD_SIZE_X / VOLUME_WORLD_SIZE_Y,
        1.0,
        VOLUME_WORLD_SIZE_Z / VOLUME_WORLD_SIZE_Y
      )
    );
  ...
  advectVelocityPass = Fn(() => {
    ...
    const turbulence = thermalTurbulence
      .add(ambientTurbulence)
      .mul(VOLUME_WORLD_SIZE_Y);
    ...
    If(dist.lessThan(teapotRadius), () => {
      ...
      const windNoisePos = uvw.add(
        vec3(0.0, uTime.mul(0.5), 0.0).div(uTurbFrequency)
      );
  ...
  const jacobi = (pressRead, pressWrite, name) =>
    Fn(() => {
      const coord = getVoxelCoord(instanceIndex);
      const uvw = coordToUVW(coord);
  ...
  emitTeapotPass = Fn(() => {
    ...
    If(
      ...
      () => {
        const coord = uvec3(
          uvw.mul(vec3(GRID_SIZE_X, GRID_SIZE_Y, GRID_SIZE_Z))
        );
        ...
        const flicker = mx_noise_float(
          vertexPos
            .mul(9.0)
            .add(vec3(0.0, uTime.negate().mul(2.5), uTime.mul(0.7)))
        )
          .mul(0.5)
          .add(0.5);

        // Baseline emission depends on temperature rate (0 if temperature is 0)
        const baseEmission = uEmitTemperature
          .greaterThan(0.0)
        ...
        If(densityVal.greaterThan(0.0), () => {
          ...
          const newAge = mix(
            currentAge,
            float(0.0),
            densityVal.div(max(newDensity, 0.001))
          );

          textureStore(
            dyeTexWriteNode,
            coord,
            vec4(newDensity, newTemp, newAge, 1.0)
          ).toWriteOnly();
        });
      }
    );
...
function init() {
  ...
  controls.target.set(
    0,
    -VOLUME_WORLD_SIZE_Y / 2 + 3.6 + VOLUME_WORLD_SIZE_Y / 2,
    0
  );
  ...
  teapotVerticesBuffer = storage(
    teapotGeometry.attributes.position,
    "vec3",
    vertexCount
  ).toReadOnly();
  ...
  const fireRamp = Fn(([t]) => {
    ...
    color.assign(
      mix(vec3(0.0, 0.0, 0.0), uFireEndColor, smoothstep(0.05, 0.35, t))
    );
  ...
  const getVolumeSample = ({ positionRay }) => {
    ...
    const noiseDistortion = texture3D(velTexA, uvw, 0)
      .xyz.div(uVolumeWorldSize)
      .mul(0.15);
    ...
    const detailNoise = snoise(
      positionRay.mul(5.5).add(vec3(0, age.mul(0.8).negate(), 0))
    );
  ...
  volumetricMaterial.scatteringNode = Fn(({ positionRay }) => {
    ...
    const smokeScattering = vec3(density)
  ...
  const volumetricShadowMesh = new Mesh(
    new BoxGeometry(
      VOLUME_WORLD_SIZE_X,
      VOLUME_WORLD_SIZE_Y,
      VOLUME_WORLD_SIZE_Z
  ...
  const floorPlane = new Mesh(
    ...
    new MeshStandardMaterial({ color: 0x111115, roughness: 0.8 })
  );
  floorPlane.rotation.x = -Math.PI / 2;
  floorPlane.position.y =
    -VOLUME_WORLD_SIZE_Y / 2 + 0.4 + VOLUME_WORLD_SIZE_Y / 2 + 0.4;
  ...
  const pointLightColor = Fn(() => {
    ...
    const r = float(1.2);
    const softAttenuation = float(1.0).div(
      distToSegment.pow(2.0).add(r.pow(2.0))
    );
    ...
    const defaultAttenuation = distToLight
      .pow(decayExponent)
      .max(0.01)
      .reciprocal();
    ...
    const currentIntensity = isVolume
      .equal(1.0)
      .select(uPointLightVolumeIntensity, uPointLightSurfaceIntensity);

    // 4. Color temperature oscillation: shift color tone slightly over time (uniform for volume)
    const colorT = uEmitTemperature
      .div(8.34)
      .mul(0.5)
      .add(0.2)
      .add(uColorNoise)
    ...
    const centerFadeFactor = smoothstep(
      0.0,
      uPointLightProjectionCenterFade,
      distXZ
    );
    ...
    const noiseCoord1 = vec3(
      P.x.mul(float(0.6).mul(freqScale)),
      uTime.mul(1.2),
      P.z.mul(float(0.6).mul(freqScale))
    );
    const projN1 = mx_noise_float(noiseCoord1).mul(0.5).add(0.5);

    const noiseCoord2 = vec3(
      P.x.mul(float(1.5).mul(freqScale)),
      uTime.mul(2.5),
      P.z.mul(float(1.5).mul(freqScale))
    );
    ...
    const projectionIntensity = projNoise.mul(
      cleanAngleNoise.mul(0.5).add(0.5)
    );

    // Fade out the noise over distance (blend to uniform 1.0)
    const noiseFadeFactor = distToSegment
      .div(uPointLightProjectionNoiseFade)
      .clamp(0.0, 1.0);
    const finalIntensity = mix(
      projectionIntensity,
      float(1.0),
      noiseFadeFactor
    );
    ...
    const radialTemp = float(1.0)
      .sub(distToSegment.div(uPointLightProjectionRadius))
      .clamp(0.0, 1.0);
    ...
    const coloredFireProj = hue(
      saturation(fireColorProj, uSaturation),
      uFireHue
    );

    // Select either uniform fire color (volume) or projected fire color (surface)
    const finalFireColor = isVolume
      .equal(1.0)
      .select(coloredFire, coloredFireProj);
  ...
  pointLight = new PointLight(0xffffff, 1, 100, 2);
  ...
  dragControls.addEventListener("drag", function () {
    ...
    teapot.position.y = Math.max(
      floorPlane.position.y - teapotMinY,
      Math.min(VOLUME_WORLD_SIZE_Y - 1.5, teapot.position.y)
    );
  ...
  keyLight = new SpotLight(0xffffff, 1000);
  ...
  const blurredVolumetricPass = gaussianBlur(
    ...
    denoiseStrength,
    1
  ).toInspector("Blurred Volumetric");
  ...
  qualityFolder
    .add(volumetricMaterial, "steps", 4, 42, 1)
    .name("Raymarch Steps");
  qualityFolder
    .add(params, "denoise")
  ...
  function updatePostProcessing() {
    ...
    const adjustedVolumetric = vec4(adjustedVolumetricRGB, volumetric.a).mul(
      0.5
    );

    const scenePassColor = scenePass
      .max(adjustedVolumetric)
      .add(adjustedVolumetric);
  ...
  volumeVisuals
    .add(uFireGlowSpread, "value", 1.0, 5.0, 0.1)
    .name("Glow Spread");
  ...
  emitterControls
    .add(uMotionBoost, "value", 0.0, 0.4, 0.01)
    .name("Movement Boost");
  emitterControls
    .add(uWindStrength, "value", 0.0, 50.0, 0.01)
    .name("Movement Wind Strength");
  emitterControls
    .add(uTeapotEmissiveIntensity, "value", 0.0, 1.0, 0.001)
    .name("Teapot Emissive");

  // Scattering & Shadows Folder
  const scatteringShadows = gui.addFolder("Scattering & Shadows");
  scatteringShadows
    .add(uAsymmetry, "value", -0.99, 0.99, 0.01)
    .name("Phase Asymmetry (g)");
  scatteringShadows
    .add(uPowderStrength, "value", 0.0, 1.0, 0.01)
    .name("Powder Effect");
  scatteringShadows
    .add(uMultiScattering, "value", 0.0, 1.0, 0.01)
    .name("Multi Scattering");
  scatteringShadows
    .add(uShadowAbsorption, "value", 0, 10)
  ...
  fluidPhysics
    .add(params, "fireLifespan", 0.5, 10.0, 0.1)
    .name("Fire Lifespan");
  fluidPhysics
    .add(params, "smokeLifespan", 1.0, 100.0, 0.5)
    .name("Smoke Lifespan");
  fluidPhysics.add(params, "turbulence", 0, 5).name("Turbulence Strength");
  fluidPhysics
    .add(uTurbulenceDecay, "value", 0.0, 1.0, 0.01)
  ...
  sceneLights
    .add(keyLight, "intensity", 0, 1500, 1)
    .name("Key Light Intensity");
  sceneLights
    .add(uPointLightVolumeIntensity, "value", 0.0, 2.0, 0.001)
    .name("Light Smoke");
  sceneLights
    .add(uLightNearIntensity, "value", 0.0, 20.0, 0.05)
    .name("Light Near Scale");
  sceneLights
    .add(uLightFarIntensity, "value", 0.0, 20.0, 0.05)
    .name("Light Far Scale");
  sceneLights
    .add(uPointLightSurfaceIntensity, "value", 0.0, 20.0, 0.001)
    .name("Light Reflection");
  sceneLights
    .add(uPointLightProjectionRadius, "value", 1.0, 30.0, 0.1)
    .name("Proj Light Radius");
  sceneLights
    .add(uPointLightProjectionFrequency, "value", 0.1, 1.0, 0.01)
    .name("Proj Light Freq");
  sceneLights
    .add(uPointLightProjectionNoiseFade, "value", 1.0, 30.0, 0.1)
    .name("Proj Noise Fade Dist");
  sceneLights
    .add(uPointLightProjectionCenterFade, "value", 0.1, 5.0, 0.05)
    .name("Proj Center Fade");
  ...
  toneMappingFolder
    .add(params, "exposure", 0.1, 2.0, 0.05)
    .name("Exposure")
    .onChange((value) => {
...
function animate() {
  ...
  if (params.simulate && params.simSpeed > 0) {
    ...
    uDt.value = simStep;
    uTurbulence.value =
      params.simSpeed > 0 ? params.turbulence / Math.sqrt(params.simSpeed) : 0;
  ...
  pointLight.distance = Math.max(
    0.01,
    40.0 * Math.max(0.2, sizeFactor) * fadeIn
  );
  ...
  uFireHue.value = MathUtils.degToRad(params.fireHue);
