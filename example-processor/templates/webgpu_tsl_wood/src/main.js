...
async function init() {
  ...
  const text_mat = new MeshStandardMaterial();
  ...
  for (let x = 0; x < WoodGenuses.length; x++) {
    for (let y = 0; y < Finishes.length; y++) {
      ...
      const cube = new Mesh(blockGeometry, material);

      cube.position.copy(getGridPosition(x, y));
      material.transformationMatrix = new Matrix4().setPosition(
        new Vector3(-0.1, 0, Math.random())
      );
      base.add(cube);
...
function add_custom_wood(text_mat) {
  ...
  createLabel(
    ...
    font,
    text_mat,
    getGridPosition(Math.round(WoodGenuses.length / 2 - 1), 5)
  );
  ...
  const customMaterial = new WoodNodeMaterial({
    ...
    cellScale: 910,
    cellSize: 0.1,
    darkGrainColor: new Color("#0c0504"),
    lightGrainColor: new Color("#926c50"),
    clearcoat: 1,
  ...
  gui
    .add(customMaterial, "largeWarpScale", 0.0, 1.0, 0.001)
    .name("largeWarpScale");
  gui
    .add(customMaterial, "largeGrainStretch", 0.0, 1.0, 0.001)
    .name("largeGrainStretch");
  gui
    .add(customMaterial, "smallWarpStrength", 0.0, 0.2, 0.001)
    .name("smallWarpStrength");
  gui
    .add(customMaterial, "smallWarpScale", 0.0, 5.0, 0.01)
    .name("smallWarpScale");
  gui
    .add(customMaterial, "fineWarpStrength", 0.0, 0.05, 0.001)
    .name("fineWarpStrength");
  gui
    .add(customMaterial, "fineWarpScale", 0.0, 50.0, 0.1)
    .name("fineWarpScale");
  gui
    .add(customMaterial, "ringThickness", 0.0, 0.1, 0.001)
    .name("ringThickness");
  gui.add(customMaterial, "ringBias", -0.2, 0.2, 0.001).name("ringBias");
  gui
    .add(customMaterial, "ringSizeVariance", 0.0, 0.2, 0.001)
    .name("ringSizeVariance");
  gui
    .add(customMaterial, "ringVarianceScale", 0.0, 10.0, 0.1)
    .name("ringVarianceScale");
  gui
    .add(customMaterial, "barkThickness", 0.0, 1.0, 0.01)
    .name("barkThickness");
  gui.add(customMaterial, "splotchScale", 0.0, 1.0, 0.01).name("splotchScale");
  gui
    .add(customMaterial, "splotchIntensity", 0.0, 1.0, 0.01)
    .name("splotchIntensity");
  ...
  gui
    ...
    .onChange((v) => customMaterial.darkGrainColor.set(v));
  gui
    .addColor({ lightGrainColor: "#926c50" }, "lightGrainColor")
    .onChange((v) => customMaterial.lightGrainColor.set(v));
  gui.add(customMaterial, "clearcoat", 0.0, 1.0, 0.01).name("clearcoat");
  gui
    .add(customMaterial, "clearcoatRoughness", 0.0, 1.0, 0.01)
    .name("clearcoatRoughness");

  const cube = new Mesh(blockGeometry, customMaterial);

  customMaterial.transformationMatrix = new Matrix4().setPosition(
    new Vector3(-0.1, 0, Math.random())
  );
