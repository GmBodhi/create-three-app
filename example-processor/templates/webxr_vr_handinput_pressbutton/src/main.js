...
let consoleMesh, torusKnot, instructionText;
...
function init() {
  ...
  consoleMesh = new Mesh(consoleGeometry, consoleMaterial);
  ...
  buttons.push(
    {
      ...
      currState: "resting",
      prevState: "resting",
      restingY: orangeButton.position.y,
      surfaceY: 0.05,
      recoverySpeed: 0.4,
    ...
    },
    {
      ...
      currState: "resting",
      prevState: "resting",
      restingY: pinkButton.position.y,
      surfaceY: 0.05,
      recoverySpeed: 0.4,
      fullPressDistance: 0.02,
      action: function () {
    ...
    },
    {
      ...
      currState: "resting",
      prevState: "resting",
      restingY: resetButton.position.y,
      surfaceY: 0.05,
      recoverySpeed: 0.4,
      fullPressDistance: 0.02,
      action: function () {
    ...
    },
    {
      ...
      currState: "resting",
      prevState: "resting",
      restingY: exitButton.position.y,
      surfaceY: 0.05,
      recoverySpeed: 0.2,
...
function setupSounds() {
  // the audio listener requires the XR camera, so the sounds can only be created within a session

  if (buttonPressSound === undefined && renderer.xr.getSession()) {
...
function calibrateConsole() {
  // position the console relative to the camera once the session has started

  if (needsCalibration && renderer.xr.getSession()) {
    const xrCamera = renderer.xr.getCamera();
    consoleMesh.position.x = xrCamera.position.x;
...
function updateFingerInput(delta) {
  buttons.forEach((button) => {
    const object = button.mesh;
...
function updateButtons() {
  buttons.forEach((button) => {
    ...
    if (button.currState == "recovering" && button.prevState != "recovering") {
      if (buttonReleaseSound) buttonReleaseSound.play();
    }

    // preserve prevState, clear currState
    // updateFingerInput() will update currState
