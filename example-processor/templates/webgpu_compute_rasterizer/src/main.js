...
async function init() {
  ...
  const lodOffsetsBuffer = storage(
    new StorageBufferAttribute(lodOffsetsData, 4),
    "uvec4",
    lods.length
  ).toReadOnly();
  ...
  const textureMap = new TextureLoader().load("textures/uv_grid_directx.jpg");
  ...
  const instanceDataBuffer = storage(
    new StorageBufferAttribute(staticInstanceData, 4),
    "vec4",
    instanceCount
  );
    ...
    If(visible, () => {
      ...
      for (let i = lods.length - 1; i > 0; i--) {
        const checkLod = float(lods[i].error)
          .mul(scale)
          .mul(pixelFactor)
          .lessThanEqual(pixelErrorThresholdUniform);
  ...
  // It renders AFTER the fullscreen quad, overlaying HW-rasterized triangles.
  {
    ...
    const hwMaterial = new NodeMaterial();
    ...
    hwMaterial.positionNode = Fn(() => {
      ...
      const vertGlobalIdx = indexBuffer.element(
        megaTriIdx.mul(3).add(localVert)
      );
    ...
    hwMesh = new Mesh(hwGeometry, hwMaterial);
  ...
  material.colorNode = Fn(() => {
    ...
    If(packedTri.shiftRight(TRIANGLE_INDEX_BITS).greaterThan(0), () => {
      ...
      const p = vec2(
        uv().x.mul(screenSize.x),
        uv().y.oneMinus().mul(screenSize.y)
      );
