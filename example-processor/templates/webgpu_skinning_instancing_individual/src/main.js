...
function createSourceVertexAttribute(geometry) {
  ...
  for (let i = 0; i < position.count; i++) {
    const amount =
      Math.max(0, 1 - Math.abs(position.getY(i) - 0.94) / 0.3) * 0.8;
...
function getSkeletonState(skeleton, instanceCount) {
  ...
  if (state === undefined) {
    ...
    const boneMatrices = new StorageBufferAttribute(
...
function createComputedMesh(source, instanceCount) {
  ...
  const sourceVertices = storage(
    createSourceVertexAttribute(geometry),
    "vec4",
    vertexCount * 3
  ).toReadOnly();
  ...
  const skinWeights = storage(
    new StorageBufferAttribute(geometry.getAttribute("skinWeight").array, 4),
    "vec4",
    vertexCount
  ).toReadOnly();
  ...
  computeSkinning = Fn(() => {
    ...
    const morphPosition = sourceVertices
      ...
      .xyz.add(
        sourceVertices
          .element(sourceOffset.add(uint(2)))
          .xyz.mul(bellyWeightsNode.element(meshInstance))
      );
    const skinVertex = bindMatrix.mul(morphPosition);
    const boneMatX = skeletonState.boneMatricesNode.element(
      boneOffset.add(skinIndex.x)
    );
    const boneMatY = skeletonState.boneMatricesNode.element(
      boneOffset.add(skinIndex.y)
    );
    const boneMatZ = skeletonState.boneMatricesNode.element(
      boneOffset.add(skinIndex.z)
    );
    const boneMatW = skeletonState.boneMatricesNode.element(
      boneOffset.add(skinIndex.w)
    );
    ...
    vertices
      ...
      .assign(vec4(instanceMatrix.mul(skinPosition).xyz, 1));
    vertices
      .element(targetOffset.add(uint(1)))
      .assign(vec4(transformNormal(skinNormal, instanceMatrix), 0));
  })()
    .compute(instanceCount * vertexCount)
    .setName("Compute Instanced Skinning");

  const meshVertex = instanceIndex
    .mul(uint(vertexCount))
    .add(vertexIndex)
    .mul(uint(2));

  material.positionNode = vertices.element(meshVertex).xyz;
  material.normalNode = transformNormalToView(
    vertices.element(meshVertex.add(uint(1))).xyz
  ).toVarying();
...
async function init() {
  ...
  const ground = new Mesh(
    ...
    new ShadowMaterial({ color: 0x5d6568, opacity: 0.3 })
  );
  ...
  new GLTFLoader().load("models/gltf/Michelle.glb", function (gltf) {
    ...
    const getBone = (name) =>
      skeleton.bones.find((bone) => bone.name.endsWith(name));
    ...
    const bellyWeights = new StorageBufferAttribute(instanceCount, 1);
    instanceMatrices = new StorageBufferAttribute(instanceCount, 16);
    bellyWeightsNode = storage(
      bellyWeights,
      "float",
      instanceCount
    ).toReadOnly();
    instanceMatricesNode = storage(
      instanceMatrices,
      "mat4",
      instanceCount
    ).toReadOnly();
...
function updateInstanceMatrix(index, variation) {
  ...
  dummy.position.set(
    variation.x,
    variation.y,
    groundFoot - foot * variation.height * variation.size
  );
  dummy.scale
    .set(variation.width, variation.depth, variation.height)
    .multiplyScalar(variation.size);
  dummy.updateMatrix();
  ...
  if (mixer) {
    ...
    for (let i = 0; i < timeOffsets.length; i++) {
      ...
      for (const state of skeletonStates.values()) {
        ...
        state.boneMatrices.array.set(
          state.skeleton.boneMatrices,
          i * state.boneCount * 16
        );
      }
    ...
    for (const state of skeletonStates.values())
      state.boneMatrices.needsUpdate = true;
