...
const _mouseOffset = new Vector3();
...
function init() {
  ...
  const dirLight2 = new DirectionalLight(0x88aaff, 0.5);
  ...
  const sizeController = gui
    .add(params, "size", 5, 500, 1)
    .onChange(function (value) {
      sculptor.setSize(value);
    });

  const strengthController = gui
    .add(params, "strength", 0, 1)
    .onChange(function (value) {
  ...
  cursorMaterial = new MeshBasicMaterial({
    color: 0xcc0000,
    side: DoubleSide,
    depthTest: false,
    depthWrite: false,
  ...
  cursorRing = new Mesh(new RingGeometry(0.95, 1, 48), cursorMaterial);
  cursorDot = new Mesh(new CircleGeometry(1, 16), cursorMaterial);
  ...
  controls.addEventListener("change", function () {
    ...
    if (cursorGroup.visible && sculptor.isSculpting() === false)
  ...
  function updateCursorPosition() {
    ...
    if (hasHit) {
      ...
      const offsetX =
        ((_pointer.x + sculptor.getSize() - rect.left) / rect.width) * 2 - 1;
