...
async function init() {
  ...
  for (const position of bigPositions) {
    const bigSphere = new Mesh(bigGeometry, bigMaterial);
  ...
  for (let ix = 0; ix < GRID; ix++) {
    for (let iz = 0; iz < GRID; iz++) {
      ...
      if (
        bigPositions.some(
          (position) =>
            Math.hypot(x - position.x, z - position.z) < BIG_RADIUS + 1
        )
      )
        continue;
      ...
      const light = new PointLight(color, 1, 9);
  ...
  gui.add(clusterInfluence, "value", 0, 0.7).name("lights per tile");
...
function updatePostProcessing() {
  ...
  let heatColor = mix(
    vec3(0.0, 0.0, 1.0),
    vec3(0.0, 1.0, 0.0),
    heatmap.mul(2.0).saturate()
  );
  heatColor = mix(
    heatColor,
    vec3(1.0, 0.0, 0.0),
    heatmap.sub(0.5).mul(2.0).saturate()
  );
...
function animate() {
  if (params.animate === true) {
    ...
    for (let i = 0; i < balls.length; i++) {
      const ball = balls[i];
      const y =
        RADIUS + (0.5 + 0.5 * Math.sin(time * 1.5 + ball.phase)) * WAVE_HEIGHT;
