...
async function init() {
  ...
  const prePass = pass(scene, camera).toInspector("Normal", (inspectNode) =>
    colorSpaceToWorking(inspectNode, SRGBColorSpace)
  );
  ...
  function addSpotLight(position, targetPosition) {
    const light = new SpotLight(0xffe09e, 40);
  ...
  addSpotLight(new Vector3(-2.5, 5, -4.8), new Vector3(-2.5, -2, -4.8));
  addSpotLight(new Vector3(2.5, 5, -4.8), new Vector3(2.5, -2, -4.8));
  addSpotLight(new Vector3(-5.3, 5, 0), new Vector3(-5.3, -2, 0));
  ...
  const floor = new Mesh(new PlaneGeometry(tilesX, tilesZ), floorMat);
  ...
  const backWall = new Mesh(new PlaneGeometry(16, 10), wallMat);
  backWall.position.set(0, 3, -5);
  scene.add(backWall);

  const leftWall = new Mesh(new PlaneGeometry(16, 10), wallMat);
  ...
  const pedestalBase = new Mesh(
    new CylinderGeometry(0.9, 1.0, 0.2, 32),
    pedestalMat
  );
  pedestalBase.position.set(0, -1.9, 0);
  scene.add(pedestalBase);

  const pedestalShaft = new Mesh(
    new CylinderGeometry(0.55, 0.65, 1.5, 32),
    pedestalMat
  );
  ...
  const pedestalTop = new Mesh(
    new CylinderGeometry(0.8, 0.7, 0.25, 32),
    pedestalMat
  );
  ...
  const torusKnot = new Mesh(
    new TorusKnotGeometry(0.5, 0.17, 128, 32),
    knotMat
  );
  ...
  function addColumn(x, z) {
    ...
    const base = new Mesh(new BoxGeometry(0.6, 0.2, 0.6), columnMat);
    base.position.y = -1.9;
    columnGroup.add(base);

    const shaft = new Mesh(new CylinderGeometry(0.18, 0.22, 5, 16), columnMat);
    shaft.position.y = 0.7;
    columnGroup.add(shaft);

    const capital = new Mesh(new BoxGeometry(0.55, 0.25, 0.55), columnMat);
  ...
  const vaseProfile = [
    ...
    new Vector2(0.32, 1.0),
    new Vector2(0.3, 1.1),
    new Vector2(0.22, 1.15),
    new Vector2(0.2, 1.2),
    new Vector2(0.22, 1.25),
    new Vector2(0, 1.25),
  ];

  const vaseMat = new MeshStandardMaterial({
    color: "#d4806a",
    roughness: 0.6,
    metalness: 0.02,
  });

  const vasePedestalBase = new Mesh(
    new CylinderGeometry(0.6, 0.7, 0.15, 32),
    pedestalMat
  );
  vasePedestalBase.position.set(-5, -1.925, -2);
  scene.add(vasePedestalBase);

  const vasePedestalShaft = new Mesh(
    new CylinderGeometry(0.35, 0.42, 1, 32),
    pedestalMat
  );
  vasePedestalShaft.position.set(-5, -1.35, -2);
  scene.add(vasePedestalShaft);

  const vasePedestalTop = new Mesh(
    new CylinderGeometry(0.55, 0.48, 0.15, 32),
    pedestalMat
  );
  vasePedestalTop.position.set(-5, -0.775, -2);
  scene.add(vasePedestalTop);

  const vase = new Mesh(new LatheGeometry(vaseProfile, 24), vaseMat);
  ...
  const backrest = new Mesh(
    new RoundedBoxGeometry(0.9, 0.6, 0.12, 4, 0.04),
    fabricMat
  );
  ...
  for (const side of [-1, 1]) {
    const armrest = new Mesh(new BoxGeometry(0.1, 0.25, 0.7), woodMat);
    armrest.position.set(side * 0.5, -1.2, 0);
    chairGroup.add(armrest);

    const armTop = new Mesh(new BoxGeometry(0.14, 0.06, 0.8), woodMat);
  ...
  for (const [lx, lz] of legPositions) {
    const leg = new Mesh(new CylinderGeometry(0.03, 0.035, 0.2, 8), woodMat);
  ...
  const tableTop = new Mesh(new CylinderGeometry(0.4, 0.4, 0.05, 24), woodMat);
  tableTop.position.y = -1.05;
  tableGroup.add(tableTop);

  const tableLeg = new Mesh(new CylinderGeometry(0.04, 0.06, 0.9, 8), woodMat);
  ...
  const cupBody = new Mesh(new CylinderGeometry(0.1, 0.08, 0.2, 16), cupMat);
  ...
  const rug = new Mesh(new BoxGeometry(6, 0.02, 5), rugMat);
  rug.position.set(0, -1.99, 0.5);
  scene.add(rug);

  const rugBorderMat = new MeshStandardMaterial({
    color: "#d4b880",
    roughness: 0.95,
    metalness: 0,
  });
  const rugBorder = new Mesh(new BoxGeometry(6.3, 0.015, 5.3), rugBorderMat);
  ...
  function addFrame(x, y, z, width, height, paintColor, rotY = 0) {
    ...
    const geo = new ExtrudeGeometry(frameShape, {
      depth: 0.12,
      bevelEnabled: true,
      bevelThickness: 0.02,
      bevelSize: 0.02,
    ...
    const frame = new Mesh(geo, frameMat);
    ...
    const canvas = new Mesh(new PlaneGeometry(width, height), paintMat);
  ...
  const sizeBox = new Box3().setFromObject(bust);
