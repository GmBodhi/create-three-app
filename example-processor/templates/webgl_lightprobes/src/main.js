...
async function init() {
  ...
  const wallMaterial = new MeshStandardMaterial({ color: 0xcccccc });
  const redMaterial = new MeshStandardMaterial({ color: 0xff0000 });
  const greenMaterial = new MeshStandardMaterial({ color: 0x00ff00 });

  // Floor
  const floor = new Mesh(new PlaneGeometry(6, 6), wallMaterial);
  floor.rotation.x = -Math.PI / 2;
  floor.receiveShadow = true;
  scene.add(floor);

  // Ceiling
  const ceiling = new Mesh(new PlaneGeometry(6, 6), wallMaterial);
  ...
  // Back wall
  const backWall = new Mesh(new PlaneGeometry(6, 5), wallMaterial);
  backWall.position.set(0, 2.5, -3);
  backWall.receiveShadow = true;
  scene.add(backWall);

  // Front wall
  const frontWall = new Mesh(new PlaneGeometry(6, 5), wallMaterial);
  ...
  // Left wall (red)
  const leftWall = new Mesh(new PlaneGeometry(6, 5), redMaterial);
  ...
  // Right wall (green)
  const rightWall = new Mesh(new PlaneGeometry(6, 5), greenMaterial);
  ...
  const objectMaterial = new MeshStandardMaterial({ color: 0xeeeeee });

  // Tall box
  const tallBox = new Mesh(new BoxGeometry(1.2, 2.5, 1.2), objectMaterial);
  ...
  // Short box
  const shortBox = new Mesh(new BoxGeometry(1.2, 1.2, 1.2), objectMaterial);
  ...
  // Light
  const light = new PointLight(0xffffff, 40);
