...
const subgroupReductionContainer =
  document.getElementById("subgroup_reduction");

document.getElementById(
  "panel-title"
).textContent += ` (Subgroup Size: ${cssSubgroupSize}, Workgroup Size: ${cssWorkgroupSize})`;
...
for (let i = 0; i < cssWorkgroupSize; i++) {
  if (i % cssSubgroupSize === 0) {
    ...
    currentSubgroupDiv.setAttribute(
      "data-label",
      `Threads ${currentSubgroupIndex * cssSubgroupSize}-${
        (currentSubgroupIndex + 1) * cssSubgroupSize - 1
      }`
    );
...
const deactivateLabelAnimation = (subgroupDiv, idx) => {
  ...
  const subgroupReductionBufferElement = document
    .getElementById(`subgroup_reduction_element_${idx}`)
    .querySelector(".thread_data");
...
const activateLabelAnimation = (subgroupDiv, idx) => {
  ...
  for (
    let i = idx * cssSubgroupSize;
    i < idx * cssSubgroupSize + cssSubgroupSize;
    i++
  ) {
    total += initialData[i];
  }
...
document
  .getElementById("subgroup-reduction-stage")
...
const leftMaterial = new MeshBasicNodeMaterial({ color: 0x00ff00 });
const rightMaterial = new MeshBasicNodeMaterial({ color: 0x00ff00 });
...
gui.add(leftEffectController, "algo", algorithms).onChange(() => {
  leftEffectController.currentAlgo.value = algorithms.findIndex(
    (val) => val === leftEffectController.algo
  );
});

gui.add(rightEffectController, "algo", algorithms).onChange(() => {
  rightEffectController.currentAlgo.value = algorithms.findIndex(
    (val) => val === rightEffectController.algo
  );
});

gui
  .add(leftEffectController, "displayMode", displayModes)
  .name("Left Display Mode")
  .onChange(() => {
    leftMaterial.colorNode =
      leftDisplayColorNodes[leftEffectController.displayMode];
    leftMaterial.needsUpdate = true;
  });
gui
  .add(rightEffectController, "displayMode", displayModes)
  .name("Right Display Mode")
  .onChange(() => {
    rightMaterial.colorNode =
      rightDisplayColorNodes[rightEffectController.displayMode];
    rightMaterial.needsUpdate = true;
  });

const debugFolder = gui.addFolder("Debug");
const elementsReducedController = debugFolder
  .add(unifiedEffectController, "elementsReduced")
  .name("Elements Reduced");
elementsReducedController.disable();
const stateLeftController = debugFolder
  .add(leftEffectController, "state")
  .name("Left Display State");
const stateRightController = debugFolder
  .add(rightEffectController, "state")
  .name("Right Display State");
stateLeftController.disable();
stateRightController.disable();
debugFolder
  .add(unifiedEffectController, "loggedBuffer", [
    "Input Buffer",
    "Input Vectorized Buffer",
    "Workgroup Sums Buffer",
    "Debug Buffer",
  ])
  .name("Buffer to Log");
debugFolder.close();
...
const createReduce1Fn = (createReduce1FnProps) => {
  const { dispatchSize, numElements, inputBuffer, workgroupSize } =
    createReduce1FnProps;
...
const createReduce2Fn = (createReduce2FnProps) => {
  const { workgroupSize, dispatchSize, numElements, inputBuffer } =
    createReduce2FnProps;
  ...
  const fnDef = Fn(() => {
    ...
    Loop(k.greaterThan(0), () => {
      If(
        invocationLocalIndex
          .lessThan(k)
          .and(invocationLocalIndex.add(k).lessThan(workgroupSize)),
        () => {
          tSum
            .element(invocationLocalIndex)
            .addAssign(tSum.element(invocationLocalIndex.add(k)));
        }
      );
      workgroupBarrier();
      k.divAssign(2);
...
const RowReduce = (rowReduceProps) => {
  ...
  Loop(block.lessThan(blockLimiter), () => {
    ...
    const startThread = blockOffset.add(
      invocationLocalIndex.mul(workPerThread)
    );
    const localThreadOffset = uint(0).toVar("localThreadOffset");
    Loop(localThreadOffset.lessThan(workPerThread), () => {
      const inputElement = inputBuffer.element(
        rowOffset.add(startThread).addLocal
      );
      ...
      if (vectorized) {
        ...
        const inputElement = inputBuffer.element(
          rowOffset.add(startThread).add(localThreadOffset)
        );
...
const WorkgroupReduce = (workgroupReduceProps) => {
  ...
  Loop(float(delta).greaterThan(1.0), () => {
    ...
    total.assign(
      select(
        invocationLocalIndex.lessThan(delta),
        subgroupSums.element(invocationLocalIndex),
        0
      ).uniformFlow()
    );
...
const createReduce3Fn = (createReduce3FnProps) => {
  ...
  const fnDef = Fn(() => {
    ...
    const currentRowSize = select(
      ...
      select(
        inputSize.greaterThan(rowOffset),
        inputSize.sub(rowOffset),
        0
      ).uniformFlow(),
      rowSize
    ).uniformFlow();
...
const createReduce4Fn = (props) => {
  ...
  const fnDef = Fn(() => {
    const perSubgroupReductionArray = createSubgroupArray(
      "uint",
      workgroupSize
    );
    ...
    const workgroupOffset = workgroupId.x.mul(
      uint(maxWorkgroupSize).mul(workPerThread)
    );
      ...
      Loop(
        ...
        () => {
          ...
          const val = select(
            ...
            inputBuffer.element(startThread),
            uvec4(0)
          ).uniformFlow();
    ...
    If(invocationSubgroupIndex.equal(uint(0)), () => {
      perSubgroupReductionArray
        .element(subgroupMetaRank)
    ...
    const alignedSize = uint(1)
      .shiftLeft(squaredSubgroupLog)
      .toVar("alignedSize");
    ...
    Loop(
      {
        ...
        name: "j",
        type: "uint",
        update: "<<= subgroupSizeLog",
      },
      () => {
        const subgroupIndex = invocationLocalIndex
          .add(1)
          .shiftLeft(offset)
          .sub(1);

        const isValidSubgroupIndex = subgroupIndex
          .lessThan(spineSize)
          .toVar("isValidSubgroupIndex");
    ...
    If(invocationLocalIndex.equal(uint(0)), () => {
      const reducedWorkgroupSum = perSubgroupReductionArray.element(
        uint(spineSize).sub(1)
      );
...
async function init(leftSideDisplay = true) {
  const effectController = leftSideDisplay
    ? leftEffectController
    : rightEffectController;
  ...
  const inputVec4BufferAttribute = new StorageInstancedBufferAttribute(
    array,
    4
  );
  const inputVectorizedStorage = storage(
    inputVec4BufferAttribute,
    "uvec4",
    vecSize
  )
    .setPBO(true)
    .setName(`CurrentVectorized_${leftSideDisplay ? "Left" : "Right"}`);
  ...
  debugFolder.add(
    functionObj,
    `Log ${leftSideDisplay ? "Left" : "Right"} Side`
  );
  ...
  const computeReduce0Fn = Fn(() => {
    ...
    inputStorage
      ...
      .addAssign(inputStorage.element(instanceIndex.add(numThreadsDispatched)));
  })();
  ...
  const displayNodes = leftSideDisplay
    ? leftDisplayColorNodes
    : rightDisplayColorNodes;
  displayNodes["Input Grid"] = Fn(() => {
    ...
    color.assign(
      getColor(inputStorage, colorChanger, gridElementWidth, gridElementHeight)
    );
  ...
  (leftSideDisplay ? leftMaterial : rightMaterial).colorNode =
    displayNodes[effectController.displayMode];
  (leftSideDisplay ? leftMaterial : rightMaterial).needsUpdate = true;

  const plane = new Mesh(
    new PlaneGeometry(1, 1),
    leftSideDisplay ? leftMaterial : rightMaterial
  );
  scene.add(plane);
  ...
  const stepAnimation = async function () {
    ...
    const stateController = leftSideDisplay
      ? stateLeftController
    ...
    if (state === "Reset") {
      ...
      timestamps[
        leftSideDisplay ? "left_side_display" : "right_side_display"
      ].innerHTML = `
