...
const mouseLightTarget = new Vector3(0, BOX_HEIGHT / 2, BOX_DEPTH / 2);
const mouseRayOriginTarget = new Vector3();
const mouseRayDirTarget = new Vector3();
...
async function init() {
  ...
  mouseLight = new PointLight(0xffffff, 80);
...
function getBoxWidth() {
  ...
  const vFov = MathUtils.degToRad(CAM_FOV / 2);
...
function createBox() {
  ...
  const walls = [
    {
      size: [boxSize.w, t, boxSize.d],
      pos: [0, -t / 2, 0],
      mat: whiteMaterial,
    },
    {
      size: [boxSize.w, t, boxSize.d],
      pos: [0, boxSize.h + t / 2, 0],
      mat: whiteMaterial,
    },
    {
      size: [boxSize.w, boxSize.h, t],
      pos: [0, hh, -hd - t / 2],
      mat: whiteMaterial,
    },
    { size: [boxSize.w, boxSize.h, t], pos: [0, hh, hd + t / 2], noMesh: true },
    {
      size: [t, boxSize.h, boxSize.d],
      pos: [-hw - t / 2, hh, 0],
      mat: redMaterial,
    },
    {
  ...
  for (const w of walls) {
    ...
    if (!w.noMesh) {
      const geo = new BoxGeometry(w.size[0], w.size[1], w.size[2]);
      const mesh = new Mesh(geo, w.mat);
...
function createBalls() {
  const sphereGeo = new SphereGeometry(BALL_RADIUS, 32, 16);
  ...
  ballsMesh = new InstancedMesh(sphereGeo, material, ballCount);
  ballsMesh.castShadow = true;
  ballsMesh.receiveShadow = true;
  scene.add(ballsMesh);

  const colors = [
    0xff4444, 0x44ff44, 0x4488ff, 0xffaa00, 0xff44ff, 0x44ffff, 0xffff44,
    0xff8844, 0x8844ff, 0x44ff88,
  ];
...
function onPointerMove(event) {
  ...
  const frontPlane = new Plane(new Vector3(0, 0, 1), -boxSize.d / 2);
...
function animate() {
  ...
  if (mouseMoving) {
    ...
    for (const body of bodies) {
      ...
      const ray = new Ray(mouseRayOrigin, mouseRayDir);
